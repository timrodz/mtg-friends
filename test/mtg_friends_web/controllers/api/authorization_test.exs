defmodule MtgFriendsWeb.API.AuthorizationTest do
  ...
  setup %{conn: conn} do
    ...
    owner_token =
      MtgFriends.Accounts.generate_user_session_token(owner) |> Base.url_encode64(padding: false)
  ...
  test "other user cannot update tournament", %{
    conn: conn,
    other_token: token,
    tournament: tournament
  } do
    conn = put_req_header(conn, "authorization", "Bearer #{token}")
    conn = put(conn, ~p"/api/tournaments/#{tournament.id}", %{name: "Hacked!"})
    assert json_response(conn, 403)["errors"]["detail"] == "Forbidden"
  end

  test "owner can update tournament", %{conn: conn, owner_token: token, tournament: tournament} do
    conn = put_req_header(conn, "authorization", "Bearer #{token}")
    conn = put(conn, ~p"/api/tournaments/#{tournament.id}", %{name: "Updated!"})
